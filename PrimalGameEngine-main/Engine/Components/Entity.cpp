...
} // anonymous namespace (encapsulating within this file)

entity create_game_entity(const entity_info& info)
{
	assert(info.transform); // all entities have a transform component
	...
	else
	{
		...
		// Resize component
		// NOTE: we don't call resize(), so the number of of memory allocation stays low
		transforms.emplace_back();
	}
	const entity new_entity{ id };
	const id::id_type index{ id::index(id) };

	// Create trasform component
	assert(!transforms[index].is_valid());
	transforms[index] = transform::create_transform(*info.transform, new_entity);
	if (!transforms[index].is_valid()) return {}; // default entity class
